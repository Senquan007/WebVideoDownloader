...
# 指定基准路径
base_folder = input("请输入下载目录：")  # 替换成你的目标路径
...
# 构造输出文件夹的路径
output_folder_path = os.path.join(base_folder, video_title)
...
# 获取视频简介和时长（假设简介和时长是连续输出的，且时长在简介之后）
...
# 搜索目录下的 XML 文件
xml_files = glob.glob(os.path.join(output_folder_path, '*.xml'))
...
if not xml_files:
    ...
    output_file = os.path.join(output_folder_path, f'{file_name}.ass')
